...
class DailyExpensesView(APIView):
    ...
    def get(self, request):
        ...
        data = [
            {"date": date, "total": total, "sub_total": sub_total}
        ]
        return Response(data, status=status.HTTP_200_OK)
...
class MonthlyExpensesView(APIView):
    ...
    def get(self, request):
        ...
        queryset = Expense.objects.filter(user=request.user, expense_date__gte=last_12_months)
...
class PaymentMethodsView(APIView):
    ...
    def get(self, request):
        ...
        queryset = Expense.objects.filter(user=request.user, expense_date__gte=start_date)
...
class CategoryExpensesView(APIView):
    ...
    def get(self, request):
        ...
        queryset = Expense.objects.filter(user=request.user, expense_date__gte=current_month)    
